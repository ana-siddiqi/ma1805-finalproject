...
function preload(){
  ...
  backgroundmusic=loadSound("choir.mp3");

  // Loads the image of the devices used in the memories
  for(let name of deviceNames){
    let img=loadImage("images/"+name+".png");
    deviceImages.push(img);
  }
}
...
for(let s of stars){
  let d=dist(mouseX, mouseY, s.x+10,s.y-10);
  if(d<40){
    currentMemory=s.memory;
//Each memory also opens up a mini poem underneath it, describing the image/video
if(currentMemory===memories[0])poem="Pixel by Pixel, my childhood slowly rebuilt itself.";
if(currentMemory===memories[1])poem="A bittersweet reuinion. Saved and cherished forever by my grandpa's camcorder.";
if(currentMemory===memories[2])poem="Technology did not raise me, however it has never left my side";
if(currentMemory===memories[3])poem="The stage felt ginormous, but the moment fit perfectly inside my mother's phone.";
if(currentMemory===memories[4])poem="A small portrait of my loved ones, captured through my father's phone";
if(currentMemory===memories[5])poem="Eight candles, one screen";
